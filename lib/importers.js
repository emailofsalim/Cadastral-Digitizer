...
(function (root, factory) {
  ...
  function parseDxf(text) {
    ...
    for (let i = 0; i < pairs.length - 1; i++) {
      if (pairs[i][0] === 9 && String(pairs[i][1]).trim() === '$INSBASE') {
        // Stop at the NEXT header variable. The scan used to run on for a
        // fixed eight pairs, straight past $INSBASE's own 10/20 and into
        // $EXTMIN's — and then overwrite with those, so the recorded origin
        // came back as the drawing's minimum corner. A round trip through
        // "shifted" mode therefore never came back in place, which is the
        // whole reason this block exists.
        let x = null; let y = null;
        for (let j = i + 1; j < pairs.length; j++) {
          if (pairs[j][0] === 9) break;
          if (pairs[j][0] === 10 && x === null) x = num(pairs[j][1]);
          if (pairs[j][0] === 20 && y === null) y = num(pairs[j][1]);
    ...
    if (insbase) {
      // Exposed, not just described: the caller can only put a shifted
      // drawing back where it belongs if it can see the origin.
      res.insBase = insbase.slice();
  ...
  /* =====================================================================
   * PDF PAGE RENDER SCALE
   * ---------------------------------------------------------------------
   * A PDF page is measured in points (1/72 inch), and rendering it at scale 1
   * gives 72 dpi — far too coarse to trace a cadastral boundary or read a plot
   * number. Rendering at an arbitrarily large scale instead is how a browser
   * tab runs out of memory on a large sheet.
   *
   * So the scale is derived from the page's own size: aim for a long edge that
   * is genuinely readable, then cap the total pixel count so an E0 drawing
   * cannot ask for a canvas the browser will refuse to allocate. Chrome's
   * practical canvas ceiling is around 16384 px on a side and a few hundred
   * megapixels of total area; the caps here sit well inside that.
   *
   * Returns the scale plus the pixel size it implies, and says whether a cap
   * was hit — the caller reports that rather than silently handing back a
   * blurrier page than was asked for.
   * =================================================================== */
  const PDF_TARGET_LONG_EDGE = 2400;   // ~200 dpi on A4: plot numbers stay legible
  const PDF_MAX_PIXELS = 40e6;         // ~40 MP, comfortably inside Chrome's limits
  const PDF_MAX_EDGE = 12000;          // and no single side near the 16384 ceiling
  const PDF_MIN_SCALE = 0.1;

  function pdfRenderScale(widthPt, heightPt, opts) {
    const o = opts || {};
    const w = num(widthPt);
    const h = num(heightPt);
    if (w === null || h === null || w <= 0 || h <= 0) {
      return { ok: false, error: 'This PDF page reports no usable size.' };
    }
    const targetEdge = o.targetLongEdge || PDF_TARGET_LONG_EDGE;
    const maxPixels = o.maxPixels || PDF_MAX_PIXELS;
    const maxEdge = o.maxEdge || PDF_MAX_EDGE;

    let scale = targetEdge / Math.max(w, h);
    // Never render smaller than the page's own natural size: a page that is
    // already large in points is not improved by shrinking it.
    if (scale < 1) scale = 1;

    let capped = null;
    if (Math.max(w, h) * scale > maxEdge) {
    ...
    if (scale < PDF_MIN_SCALE) scale = PDF_MIN_SCALE;
    ...
    return {
      ...
      capped,
      note: capped
        ? 'This page is very large, so it was rendered at a reduced scale to stay within the browser’s memory limits. Zoom in to trace; the detail that is there is at full resolution for this scale.'
        : null,
    };
  }

  /* A PDF begins "%PDF-" whatever the file is called or what MIME type the
   * picker reported. Extensions and MIME types are both routinely wrong on
   * files that arrive by email or messaging app (brief I). */
  function looksLikePdfBytes(bytes) {
    if (!bytes || bytes.length < 5) return false;
    const u8 = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
    // Some files carry leading whitespace or a BOM before the header.
    const limit = Math.min(u8.length - 4, 1024);
    for (let i = 0; i <= limit; i++) {
      if (u8[i] === 0x25 && u8[i + 1] === 0x50 && u8[i + 2] === 0x44
        && u8[i + 3] === 0x46 && u8[i + 4] === 0x2d) return true;   // %PDF-
    }
    return false;
  }
  ...
  return {
    ...
    readZipEntries, extractKmlFromKmz, inflateRaw, utf8Decode,
    // GeoJSON
    parseGeoJson,
    // CSV
    ...
    // PDF page rendering
    pdfRenderScale, looksLikePdfBytes,
    PDF_TARGET_LONG_EDGE, PDF_MAX_PIXELS, PDF_MAX_EDGE,
    // dispatch
