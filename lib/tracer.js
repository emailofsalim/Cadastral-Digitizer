...
(function (root, factory) {
  ...
  function refineEdgeAlongNormal(raster, x, y, nx, ny, options) {
    ...
    // WHERE the crossing is, to better than a pixel. The samples either side
    // give the colour distance on each side of the threshold, and the boundary
    // is where that distance would cross it — so a linear interpolation between
    // the two puts the vertex on the edge rather than on the nearer pixel
    // centre. Opt-in: without it the answer is the pixel midpoint, exactly as
    // 17.5.0 computed it, so nothing that relied on that changes.
    let offset = (ts[cut - 1] + ts[cut]) / 2;
    if (o.subpixel && dists) {
      const dIn = dists[cut - 1];
      const dOut = dists[cut];
      const span = dOut - dIn;
      // Guard the degenerate case: with no gradient across the pair there is
      // nothing to interpolate and the midpoint is the honest answer.
      if (Math.abs(span) > 1e-9) {
        let f = (tolSq - dIn) / span;
  ...
  /* =====================================================================
   * A WHOLE RING AGAINST THE PIXELS  (17.6)
   * ---------------------------------------------------------------------
   * Moving existing corners can only ever make a polygon a better version of
   * the polygon it already is. A four-corner box traced over a parcel with a
   * kink in one side has nowhere to put the kink, however well each corner is
   * placed — the shape needs a corner it does not have.
   *
   * So this adds them, by ADAPTIVE SUBDIVISION: probe the middle of each edge,
   * and where the real boundary is measurably off the straight line between
   * its endpoints, put a corner where the boundary actually is and recurse into
   * the two halves. It is Douglas-Peucker run backwards — instead of dropping
   * vertices that add no shape, it adds the ones that do.
   *
   * The decision to insert reuses refineEdgeAlongNormal's refusals wholesale,
   * with insertTolerancePx as its settle distance: a midpoint whose boundary is
   * already on the chord reports "settled" and nothing is inserted, and an
   * ambiguous or unreadable probe reports nothing at all. Vertices therefore
   * appear only where the pixels positively demand one.
   *
   * FOUR SEPARATE THINGS STOP IT RUNNING AWAY, because vertex explosion is the
   * failure mode that would make this unusable on a real sheet:
   *
   *   insertTolerancePx  a boundary already followed within tolerance gets no
   *                      new corner — so a second pass over the same view
   *                      inserts nothing, and the process converges;
   *   minSpacingPx       no corner may be planted near an existing one,
   *                      which bounds the density independently of tolerance;
   *   maxDepth           recursion is finite by construction;
   *   maxInsert          a hard budget per pass, so even a pathological raster
   *                      cannot produce more than a known number of corners.
   *
   * Returns a NEW ring; nothing is mutated.
   * =================================================================== */
  function refineRingToEdge(raster, ring, options) {
    const o = options || {};
    const insertTolerancePx = Math.max(0.2, Number(o.insertTolerancePx || 2));
    const minSpacingPx = Math.max(1, Number(o.minSpacingPx || 6));
    const maxDepth = Math.max(0, Math.round(Number(o.maxDepth == null ? 3 : o.maxDepth)));
    const maxInsert = Math.max(0, Math.round(Number(o.maxInsert == null ? 12 : o.maxInsert)));
    const maxVertices = Math.max(3, Math.round(Number(o.maxVertices || 400)));
    const probe = Object.assign({}, o, { subpixel: true });

    const inBounds = (x, y) => {
      const pad = Number(o.searchPx || 12);
      return x >= pad && y >= pad && x <= raster.width - pad - 1 && y <= raster.height - pad - 1;
    };

    // ---- phase 1: move the corners that are already there -----------------
    const work = ring.map((p) => p.slice());
    let moved = 0;
    for (let i = 0; i < ring.length; i++) {
      if (!inBounds(ring[i][0], ring[i][1])) continue;
      const n = ringVertexNormal(ring, i);
      if (!n) continue;
      const r = refineEdgeAlongNormal(raster, ring[i][0], ring[i][1], n[0], n[1], probe);
      if (!r.ok) continue;
      work[i] = [ring[i][0] + r.dx, ring[i][1] + r.dy];
      moved++;
    }
    if (!o.insert || !maxInsert) return { ring: work, moved, inserted: 0 };

    // ---- phase 2: add the corners the shape is missing --------------------
    let budget = maxInsert;
    const subdivide = (a, b, depth) => {
      if (depth >= maxDepth || budget <= 0) return [];
      const dx = b[0] - a[0], dy = b[1] - a[1];
      const len = Math.hypot(dx, dy);
      // Too short to hold another corner without breaching minimum spacing.
      if (len < 2 * minSpacingPx) return [];
      const mx = a[0] + dx / 2, my = a[1] + dy / 2;
      if (!inBounds(mx, my)) return [];
      const nx = -dy / len, ny = dx / len;
      const r = refineEdgeAlongNormal(raster, mx, my, nx, ny,
        Object.assign({}, probe, { settlePx: insertTolerancePx }));
      if (!r.ok) return [];                       // settled, ambiguous, or unreadable
      const p = [mx + r.dx, my + r.dy];
      budget--;
      const left = subdivide(a, p, depth + 1);
      const right = subdivide(p, b, depth + 1);
    ...
    for (let i = 0; i < work.length; i++) {
      ...
      out.push(a);
      if (out.length >= maxVertices) continue;
      for (const p of subdivide(a, b, 0)) {
        // Minimum spacing is enforced against what has actually been kept, not
        // against the original ring — so a chain of inserts cannot bunch up.
        const prev = out[out.length - 1];
        if (Math.hypot(p[0] - prev[0], p[1] - prev[1]) < minSpacingPx) continue;
        if (Math.hypot(p[0] - b[0], p[1] - b[1]) < minSpacingPx) continue;
        if (out.length >= maxVertices) break;
        out.push(p);
        inserted++;
      }
    }
    return { ring: out, moved, inserted };
  }
  ...
  return {
    ...
    refineEdgeAlongNormal, ringVertexNormal, refineRingToEdge,
